...
class UserHandler(BaseHandler):
    def process_get_request(self, parameters, kwargs):
        response_dict: dict[str, typing.Union[bool, str, list, dict]] = {
            "success": False
        }
    ...
    def process_post_request(self, parameters, body, kwargs):
        ...
        if user_object:
            response_dict["success"] = True
            response_dict["data"] = {"user": user_object.to_json()}
        else:
...
class CommandHandler(BaseHandler):
    def process_get_request(self, parameters, kwargs):
        ...
        if search_text is not None:
            ...
            response_dict["data"] = {}
            response_dict["data"]["totalPages"] = paginator.page_range.stop - 1
            response_dict["data"]["currentPage"] = page_number
            ...
            response_dict["data"].update({"commands": []})

            print(response_dict["data"])
            print(current_qs)
            for a_obj in current_qs:
    ...
    def process_post_request(self, parameters, body, kwargs):
        ...
        result = command_rest_models.Command.add(
            name, text, description, catagory, user_email
        )
        if result:
            response_dict["success"] = True
            response_dict["data"] = {"command": [result.to_json()]}
